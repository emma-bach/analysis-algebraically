...
\begin{document}
	...
	\part{Topology}
		...
		\begin{proof}
			...
			\end{enumerate}
		\end{proof}
		\begin{importanttheorem}{A function only continuous at the irrationals}{Thomae's function}{} 
		$T : \bR \to \bR$, defined as
			\begin{align*}
				T(x) =
				\begin{cases}
	...
	\chapter{Measure Theory}
		...
		\end{proposition}
		\begin{proof}
			We can decompose any disjoint union of sets further by adding the empty set, therefore our measure should be defined on the empty set. Given any disjoint sets, we want the measure of their countable union to be equal to the sum of their measures, therefore the measure of the countable union should be well-defined in the first place.
		\end{proof}
		...
		\end{corollary}
		\begin{proof}
			We can add the empty set to any disjoint union of sets without changing the resulting set, therefore the measure of the empty set should not change the sum, i.e. it should be zero.
		\end{proof}
		...
		\end{corollary}	
		\begin{proof}
			We have $B = A \sqcup (B \setminus A)$, implying $\mu(B) = \mu(A) + \mu(B \setminus A) \geq \mu(A)$.
		\end{proof}
	...
	\chapter{Lebesque Integration}
		...
		\end{corollary}
		\begin{importantcorollary}
			{Comparison Test for the Lebesque Integral}{}
		...
		\section{Comparing Lebesque Integration with Riemann Integration}
		\begin{theorem}
	...
	\chapter{Integration over Immersed Manifolds}
		...
		\pagebreak
		\begin{corollary}
			The volume of the open unit ball
			\begin{align*}
				B^n_1(0) = \lr{\vx \in \bR^n \mid \norm{\vx}_2 < 1}.
			\end{align*}
			is given by:
			\begin{align*}
				\lambda^n(B^n_1(0))
				...
				\begin{cases}
					\frac{\pi^k}{k!} & n = 2k\\
					\frac{\pi^k}{\prod_{i = 1}^k \frac{1}{2} + k - i} & n = 2k + 1
				\end{cases}
			\end{align*}
		\end{corollary}
		In particular:
		...
		\end{enumerate}
		\begin{proof}
				First, we want to show by induction that the volume of an $n$-dimensional ball is proportional to the $n$-th power of its radius. The case $n = 1$ is trivial, since we have 
				\begin{align*}
					\lambda(B_r^1(0))
					&= \lambda((-r,r))
					= r \cdot \lambda((-1,1))
					= r \cdot \lambda(B_1^1(0))
				\end{align*}
				Now, by Cavalieri's Principle, we get:
				\begin{align*}
					\lambda^n(B^n_r(0))
					&=
					...
					&=
					\int_{(-r,r)} \lambda^{n-1}\lr{x \in \bR^{n-1} \mid \norm{x}_2 < \sqrt{r^2-y^2}} ~dy\\
					&=
					\int_{(-r,r)} \lambda^{n-1}\lr{x \in \bR^{n-1} \mid \norm{x}_2 < r\sqrt{1-\lr(\frac{y}{r})^2}} ~dy\\
					&\overset{IH}{=}
					r^{n-1} \cdot \int_{(-r,r)} \lambda^{n-1}\lr{x \in \bR^{n-1} \mid \norm{x}_2 < \sqrt{1-\lr(\frac{y}{r})^2}} ~dy\\
					&=
					r^n \cdot \int_{-r}^r \lambda^{n-1}\lr{x \in \bR^{n-1} \mid \norm{x}_2 < \sqrt{1-\lr(\frac{y}{r})^2}} \cdot \frac{1}{r} ~dy\\
					&\overset{g(x) := \frac{x}{r}}{=}
					r^n \cdot \int_{\frac{-r}{r} = -1}^{\frac{r}{r} = 1} \lambda^{n-1}\lr{x \in \bR^{n-1} \mid \norm{x}_2 < \sqrt{1-u^2}} ~du\\
					&=
					r^n \cdot \lambda^n(B^n_1(0)),
				\end{align*}
				which completes the induction.
				\newpar
				Now, using this knowledge, we can extract a recurrence relation from one of our intermediate steps:
				\begin{align*}
					\lambda^n(B^n_1(0))
					&=
					\int_{-1}^1 \lambda^{n-1}\lr{x \in \bR^{n-1} \mid \norm{x}_2 < \sqrt{1-y^2}} ~dy\\
					&=
					\int_{-1}^1 \lr(\sqrt{1-y^2})^{n-1} \cdot \lambda^{n-1} \lr{x \in \bR^{n-1} \mid \norm{x}_2 < 1} ~dy\\
					&=
					\lambda^{n-1}(B^{n-1}_1(0)) \cdot \int_{-1}^1 \lr(\sqrt{1 - y^2})^{n-1} ~dy\\
				\end{align*}
				...
				And, recalling the fact that $\lambda^0$ is the counting measure, we also know that \begin{align*}
					\lambda^0(B_1^0(0)) = \lambda^0(\lr{0}) = 1
				\end{align*}
				Thus, at long last, we get:
				\begin{align*}
					\lambda^{2k}(B^{2k}_1(0)) 
