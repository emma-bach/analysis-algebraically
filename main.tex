...
\begin{document}
	...
	\chapter{Differentiation in Normed Vector Spaces}
		...
		\begin{corollary}
			...
			\begin{align*}
				\div~ (\id) = d,
			\end{align*}
			which means that, in cases where the notation is less formal, we have
			\begin{align*}
				\div~ x = d.
			\end{align*}
		\end{corollary}
	...
	\end{importantdefinition}
	\begin{importanttheorem}{Empty interior does not imply zero measure}{}
		The Smith-Volterra-Cantor set has empty interior but positive measure.
	\end{importanttheorem}
	...
	\chapter{Lebesque Integration}
		...
		\begin{importanttheorem}{Fatou's Lemma}{}
			...
			\end{align*}
		\end{importanttheorem}
		\begin{importanttheorem}{Lebesque}{}
		...
		\begin{proof}
			...
			\begin{enumerate}
				...
				\item[$p > 1$:] Assume that $\norm{f}_p \cdot \norm{g}_q > 0$. Then, applying the Young inequality with $a = \abs{f(x)}/\norm{f}_p$ and $b = \abs{g(x)}/\norm{g}_q$, we get
				\begin{align*}
					\frac{f(x)g(x)}{\norm{f}_p\norm{g}_q}
					&\leq
	...
	\section{Alternative coordinate systems on $\bR^2$ and $\bR^3$}
		...
		\begin{importanttheorem}{Divergence Theorem / Gauss's Theorem}{}
			...
			\end{align*}
		\end{importanttheorem}
		\begin{importanttheorem}{Green's First Identity}{}
			...
			\begin{align*}
				&\int_\Omega f(x) \cdot \Delta g(x) + \scalar{\grad f(x)}{\grad g(x)} ~d\lambda^n(x)\\
		...
		\begin{proof}
			...
			\begin{align*}
				~&f(x) \cdot \Delta g(x) + \scalar{\grad f(x)}{\grad g(x)}\\
				=~&f(x) \cdot \div(\grad g(x)) + \scalar{\grad f(x)}{\grad g(x)}\\
				=~&\div(f(x) \cdot \grad g(x))\\
			\end{align*}
			...
			\begin{align*}
				...
				\int_\Omega f(x) \cdot \Delta g(x) + \scalar{\grad f(x)}{\grad g(x)} ~d\lambda^n(x)\\
				=&
				\int_\Omega \div (f(x) \cdot \grad g(x)) ~d\lambda^n(x)\\
				=&
				\int_{\partial \Omega} \scalar{f(x) \cdot \grad g(x)}{\nu_\Omega(x)} ~d\omega(x)\\
				= &\int_{\partial \Omega} f(x) \cdot \scalar{\nu_\Omega(x)}{\grad g(x)} ~d\omega(x) 
			\end{align*}
		...
		\begin{importanttheorem}{Green's Second Identity}{}
			...
			\begin{align*}
				&\int_\Omega f(x) \cdot  \Delta g(x) - g(x) \cdot \Delta f(x) ~d\lambda^n\\ = &\int_{\partial \Omega} f(x) \cdot \scalar{\nu_\Omega(x)}{\grad g(x)}
				-
				g(x)
				\cdot
				\scalar{\nu_\Omega(x)}{\grad f(x)}
