...
\begin{document}
	...
	\part{Topology}
		...
		\end{corollary}
		\begin{proposition}
			Let $f$ be a function between complete metric spaces. Then the set of continuitity of $f$ is $G_\delta$ and the set of discontinuities of $f$ is $F_\sigma$.
		\end{proposition}
		\begin{corollary}
	...
	\chapter{Measure Theory}
		...
		In the next section, we will introduce various types of \tit{set algebras} - algebraic structures on the power set of a given set. These will be important because we can define different variants of a "measure function" on these set algebras, which will provide us with weaker or stronger definitions based on the set algebra they are defined on.
		\newpar
		However, in order to gain a full birds-eye view of these definitions, we will first introduce the more general notion of \tit{boolean algebras}:
		...
		\end{definition}
		\begin{corollary}
			A boolean algebra is a bounded semilattice such that meet and join are distributive over each other and such that complements exist.
		\end{corollary}
