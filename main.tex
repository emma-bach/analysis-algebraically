...
\begin{document}
	...
	\chapter{Measure Theory}
		...
		\begin{theorem}
			...
			\begin{align*}
				...
				:= \lr{ E \bigmid E = \bigcup_{\alpha \in J} A_\alpha, J \subset I}
			\end{align*}
	...
	\end{proof}
	\begin{theorem}
		\theoremname{Simple criteria for measurability)} Let $(X, \cA)$ be a measureable space. Let $f : X \to \barR$. Then the following are equivalent:
		\begin{enumerate}
	...
	\chapter{Lebesque Integration}
		...
		\end{theorem}
		\begin{theorem}
			\theoremname{Tao, Jordan Measure, Exercise 25}
			A bounded function $f : [a,b] \to \bR$ is Riemann integrable if and only if it is Lebesque integrable using the Jordan measure.
		\end{theorem}
		...
		\begin{theorem}
			...
			\end{align*}
			is a measure, which we call the \tbf{density of $\nu$ with respect to $\mu$} and which we denote $\mu_\theta$.
		\end{theorem}
		\begin{corollary}
		...
		\end{theorem}
		\begin{proof}
			(i) $\implies$ (ii) is the Radon-Nikodym theorem. (ii) $\implies$ (iii) follows from absolute continuity of the Lebesque integral. (iii) $\implies$ (i) follows immediately from the definition of $\ll$.
		\end{proof}
		\begin{theorem}
			The Radon-Nikodym theorem also holds if $\mu$ and $\nu$ are only $\sigma$-finite.
		\end{theorem}
		\begin{definition}
	...
	\chapter{Integration over Immersed Manifolds}
		...
		\end{corollary}
		\begin{proof}
			Apply the integral transformation equation to $f = \chi_{\phi(A)}$.
		\end{proof}
		\begin{example}
			\theoremname{Two-Dimensional Bell Curve}
			We want to find the area under the Gaussian bell curve, given by
			\begin{align*}
				\int_\bR e^{-x^2} ~dx
			\end{align*}
			To do this, we add an additional dimension and exploit the resulting rotational symmetry.
			Let $f : \bR^2 \to \bR$ with
			\begin{align*}
				f(x,y) = e^{-(x^2 + y^2)}
			\end{align*}
		\end{example}
